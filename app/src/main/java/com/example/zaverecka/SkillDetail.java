...
public class SkillDetail extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Decor decorator = new Decor(Color.BLUE, progressDates); // nastaveni "oznaceni"
        ...
        // vybirani casu kdy se ma poslat notifikace
        casPicker = findViewById(R.id.timePicker);
        casPicker.setIs24HourView(true); // 24hodinový formát

        // nacitani stavu notifikace - jestli je off nebo on
        SharedPreferences prefs = getSharedPreferences("notif_prefs", MODE_PRIVATE);
        boolean notifikaceZapnuta = prefs.getBoolean("notifikaceZapnuta", false);

        TimePicker timePicker = findViewById(R.id.timePicker);
        Switch notificationSwitch = findViewById(R.id.notificationSwitch);
        TextView nextNotificationInfo = findViewById(R.id.nextNotificationInfo);

        // kdyz to tu neni, tak default je vypnuto i kdyz je zapnuto a je to hnus ble
        notificationSwitch.setChecked(notifikaceZapnuta);

        // jestli jsou true tak ukaze kdy se pusti dalsi notifikace
        if (notifikaceZapnuta) {
            ...
            String timeText = String.format(Locale.getDefault(), "Další upozornění přijde: %tH:%tM, %tA %<td.%<tm.%<tY",
                    calendar, calendar, calendar);
            nextNotificationInfo.setText(timeText);
        } else {
            nextNotificationInfo.setText("Notifikace jsou vypnuté.");
        }

        SharedPreferences.Editor editor = prefs.edit();

        // reakce na zmenu vypinace
        notificationSwitch.setOnCheckedChangeListener(new Switch.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if (isChecked) {
                    // zapnuti notifikace
                    UpozorneniHelper.scheduleDailyNotification(SkillDetail.this, hour, minute);
                    Toast.makeText(SkillDetail.this, "Notifikace nastavena na " + hour + ":" + (minute < 10 ? "0" + minute : minute), Toast.LENGTH_SHORT).show();

                    // ulozeni do shared pref. pro main
                    editor.putBoolean("notifikaceZapnuta", true);
                    editor.putInt("hour", hour);
                    editor.putInt("minute", minute);
                    editor.apply();

                    // matematika pro vypocet dalsi notifikace
                    Calendar calendar = Calendar.getInstance();
                    calendar.set(Calendar.HOUR_OF_DAY, hour);
                    calendar.set(Calendar.MINUTE, minute);
                    calendar.set(Calendar.SECOND, 0);
                    ...
                    // vypnuti notif.
                    UpozorneniHelper.cancelNotification(SkillDetail.this);
                    Toast.makeText(SkillDetail.this, "Notifikace vypnuta", Toast.LENGTH_SHORT).show();

                    editor.putBoolean("notifikaceZapnuta", false);
                    editor.apply();
    ...
    private void loadCalendarHighlights() {
        ...
        Decor decorator = new Decor(Color.BLUE, progressDates);
